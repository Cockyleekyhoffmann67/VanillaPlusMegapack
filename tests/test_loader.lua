...
local names = {pack, 'mods/cowboybingus/better_stratagem_bounce',
    ...
    'mods/cowboybingus/sentry_aim_retention', 'mods/cowboybingus/corpse_collision_repair'}
local folders = {'', 'BetterStratagemBounce', 'HellpodSteeringUnlocked', 'ReinforcementBeaconsFixed',
    'ConsistentVaulting', 'ShallowWaterDiving', 'SentryAimRetention', 'EnemyCollisionSynchronized'}
  ...
  for _, installed_pack in ipairs({false, true}) do
    for failure = 0, #names * 2 do
        ...
        if installed_loader then
            assert(env.CowboyBingusModLoader.version >= 10 and env.CowboyBingusModLoader.api == 1)
            ...
            if installed_pack and failure ~= 1 and failure ~= #names + 1 then
                assert(identity.name == 'Vanilla Plus Megapack' and identity.revision == 'megapack-v3')
