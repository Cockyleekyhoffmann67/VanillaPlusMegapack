...
for rva,ptr in pairs({[0x276c190]=pm,[0x276c3d0]=mode,[0x276f0c0]=owner,[0x276ca30]=am,[0x276c798]=dm,[0x276c2c8]=sm,[0x276c280]=mm}) do p(region(game+rva,8),0,ptr) end
for rva,v in pairs({[0x211c950]=2,[0x211c50c]=0.9,[0x211c240]=0.4}) do f(region(game+rva,4),0,v) end
...
map(region(owner+0xf21a88,20),0,0x53000000,9,1)
...
local entityptrs=region(0x53300000,16);p(entityptrs,0,owner+0xf31ad8);p(entityptrs,8,owner+0xf31ad8+24);p(drown,56,0x53300000)
...
local resource=region(0x54000000,120*16+6*64);p(region(owner+0xf119e0,8),0,0x54000000)
ffi.copy(resource,'\151\250\077\041\077\051\028\077',8);u(resource,8,5)
u(resource,120*16+5*64,0x4a182741);f(resource,120*16+5*64+4,-1.3)
...
local function reset()
    ...
    u(avatars,local_ctl+0xf88,0);u(avatars,local_ctl+0xf8c,4)
    f(avatars,local_ctl+0xb84+8,0);f(avatars,local_ctl+0xb84+12,0)
    enables[2]=1;enables[3]=1
    f(water,8,-0.3999999761581421);f(water,16,0.011);f(water,20,4.989);f(water,24,-0.9994804859161377)
...
reset();assert(apply());f(avatars,local_ctl+0xb84+12,0.5);assert(apply())
...
for _,kind in ipairs({'prone','swim','ragdoll','deep','old','landing','drowned','disabled','other_owner'}) do
    ...
    elseif kind=='swim' then u(avatars,local_ctl+0xf88,0x10000000)
    elseif kind=='ragdoll' then u(avatars,local_ctl+0xf8c,6)
...
reset();enables[3]=0;f(water,24,0);f(water,16,0);assert(apply());assert(state.protected==1)
near(number(water,8),-1.3) -- zero height while disabled is not treated as deep water
f(avatars,local_ctl+0xb84+8,2);assert(apply());near(number(water,8),-0.4)
...
local missing_pointers={
    {game+0x276c3d0,mode},{game+0x276c190,pm},{pm+0xe8,0x22000000},
    {game+0x276f0c0,owner},{game+0x276ca30,am},{game+0x276c798,dm},
    {dm+56,0x53300000},{dm+64,0x53400000},{dm+72,0x53500000},
    {game+0x276c2c8,sm},{sm+56,0x53700000},{game+0x276c280,mm},
    {mm+0x48d8,0x53900000},{owner+0xf119e0,0x54000000},
}
...
reset();p(locate(game+0x276c3d0,8),0,123);assert(not pcall(apply));assert(writes==0)
p(locate(game+0x276c3d0,8),0,mode)
reset();f(resource,120*16+5*64+4,-1.1);assert(not pcall(apply));assert(writes==0)
f(resource,120*16+5*64+4,-1.3)
print('PASS: captured startup regression, dry/wet landings, deep water, ordinary prone, ragdoll, timeout, local index 1, ownership, transitions and write failures')
