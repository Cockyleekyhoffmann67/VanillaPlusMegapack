...
1. Close the game. Replace the old standalone package with `Enemy-Collision-Synchronized-v2.9.zip`, or update to `Vanilla-Plus-Megapack-v9.zip` and enable its Enemy Collision Synchronized option. Keep Bingus Shared Loader v12 or newer for the megapack. Purge / Deploy, then launch normally.
