...
The package and compiled-loader tests cover all 4096 selections, including none, each singleton, and all twelve. Optional backend harnesses import the final ZIP into isolated Arsenal 0.36.0 and HD2MM fixtures, deploy all 4096 selections, compare every deployed triplet byte for byte, and verify purge, re-enable and removal. No live game directory or manager profile is used. See CONTRIBUTING.md for commands.
